...
begin
  ...
  try
    vClientFornecedor.LoadData(['ID'], [cdsMain.FieldByName('ID').AsString]);
    if not vClientFornecedor.Delete then
      raise Exception.Create(vClientFornecedor.Errors[vClientFornecedor.Errors.Count -1]);
    LoadData();
  finally
...
begin
  if not TGlobal.Controller.Connection.Connected then
    Exit;
